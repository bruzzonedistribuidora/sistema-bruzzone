...
export const fetchLatestFinancingRates = async (platformName: string): Promise<{installments: CreditInstallment[], sources: {title: string, uri: string}[]}> => {
  try {
    const model = 'gemini-3-flash-preview';
    const prompt = `Busca las tasas de interés, recargos o coeficientes de financiación vigentes hoy para cobros con tarjeta de crédito en la plataforma "${platformName}" en Argentina. 
...
export const searchVirtualInventory = async (query: string): Promise<Product[]> => {
  try {
    ...
    // Fix: call generateContent on ai.models, not on the model name string
    const response = await ai.models.generateContent({
      model,
      contents: prompt,
      config: {
