...
import { 
    ...
    Network, ShieldCheck, Zap, RefreshCw, Cpu, UploadCloud, CloudDownload
} from 'lucide-react';
...
const CloudHub: React.FC = () => {
    ...
    const handleSaveVault = () => {
        ...
        setConnStatus('IDLE');
        // También iniciar una sincronización desde remoto al configurar la bóveda
        syncService.syncFromRemote();
    };

    const handlePushAllData = async () => {
        if (!confirm("¿Está seguro que desea forzar la SUBIDA de TODOS sus datos locales a la nube? Esto sobrescribirá datos si hay conflictos.")) return;
    ...
    const handleFetchAllData = async () => {
        if (!confirm("¿Está seguro que desea forzar la DESCARGA de TODOS los datos de la nube? Esto podría sobrescribir sus datos locales.")) return;
        setIsPushingAll(true); // Usamos el mismo loader por simplicidad
        ...
        <div className="p-8 max-w-7xl mx-auto h-full space-y-8 animate-fade-in bg-slate-50 overflow-y-auto pb-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm space-y-6">
                        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b pb-4 flex items-center gap-2"><Network size={14} className="text-indigo-600"/> Gestión de Datos</h3>
                        <div className="space-y-3">
                            <button 
                                onClick={handlePushAllData} 
                                disabled={isPushingAll}
                                className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[1.2rem] font-black uppercase text-[10px] tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50">
                                {isPushingAll ? <RefreshCw className="animate-spin" size={16}/> : <UploadCloud size={16}/>}
                                {isPushingAll ? 'Enviando Datos...' : 'Enviar TODO Local a Nube'}
                            </button>
                            <button 
                                onClick={handleFetchAllData} 
                                disabled={isPushingAll}
                                className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-[1.2rem] font-black uppercase text-[10px] tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50">
                                {isPushingAll ? <RefreshCw className="animate-spin" size={16}/> : <CloudDownload size={16}/>}
                                {isPushingAll ? 'Recibiendo Datos...' : 'Descargar TODO de Nube'}
                            </button>
                        </div>
                    </div>
