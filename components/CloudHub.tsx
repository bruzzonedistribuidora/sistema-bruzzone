...
import { 
    ...
    ArrowUpRight, History, CheckCircle2, Cloud, Info
} from 'lucide-react';
...
const CloudHub: React.FC = () => {
    ...
    useEffect(() => {
        const handlePulse = (e: any) => {
            ...
            if (detail.status === 'OK') {
                ...
                setTerminals(list);
                setRecentLogsCount(prev => prev + (detail.logsCount || 0));
    ...
    const handleSaveVault = () => {
        ...
        setConnStatus('IDLE');
        alert("Configuración enviada. El sistema creará la bóveda automáticamente en el próximo pulso de red.");
    };
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto h-full space-y-8 animate-fade-in bg-slate-50 overflow-y-auto pb-32">
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <p className={`font-bold text-[10px] uppercase tracking-widest mt-2 flex items-center gap-2 ${connStatus === 'CONNECTED' ? 'text-green-400' : 'text-slate-500'}`}>
                                    ...
                                    {connStatus === 'CONNECTED' ? 'MODO SIMULTÁNEO ACTIVO' : connStatus === 'ERROR' ? 'ERROR DE CONEXIÓN CLOUD' : 'ESPERANDO VINCULACIÓN...'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-8 bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-6">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl font-black text-white outline-none focus:border-indigo-500 uppercase text-center" value={vaultId} onChange={e => setVaultId(e.target.value)} placeholder="EJ: BRUZZONE-SUR-2024" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Nombre de esta PC</label>
                                    <input className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl font-black text-white outline-none focus:border-indigo-500 uppercase text-center" value={terminalName} onChange={e => setTerminalName(e.target.value)} placeholder="EJ: CAJA-01" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 flex-1 overflow-y-auto custom-scrollbar">
                                {terminals.length === 0 ? (
                                    ...
                                    <div key={idx} className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                                        ...
                                        <span className="text-[8px] font-black uppercase text-slate-500">{t.isLocal ? 'ESTA PC' : 'EN VIVO'}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {/* SECCION PLAN B: CLONACION MASIVA (140K ARTICULOS) */}
            <div className="bg-white p-10 rounded-[3.5rem] border border-slate-200 shadow-xl space-y-10 relative overflow-hidden">
                ...
                <div className="flex items-start gap-6 border-b border-slate-100 pb-8">
                    ...
                    <div>
                        <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Plan B: Carga de Base Maestra (Clonación)</h3>
                        <p className="text-slate-400 text-sm font-medium mt-1">Usa este método para mover los 140.000 artículos de una PC a otra sin depender de Internet.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                    <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200 space-y-6 group hover:border-indigo-300 transition-all">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed font-medium">Exporta toda la ferretería configurada, marcas y precios en un solo archivo comprimido.</p>
                        <button 
                            ...
                            {isProcessing ? <RefreshCw className="animate-spin" size={18}/> : <FileJson size={18}/>} Exportar Base Maestra
                        </button>
                    </div>
                    ...
                    <div className="bg-indigo-50/50 p-8 rounded-[2.5rem] border border-indigo-100 space-y-6 group hover:border-indigo-300 transition-all">
                        <div className="flex items-center gap-4">
                            ...
                            <h4 className="font-black text-slate-800 uppercase text-sm">En PC Secundaria</h4>
                        </div>
                        <p className="text-xs text-slate-600 leading-relaxed font-medium">Selecciona el archivo generado para clonar el inventario completo instantáneamente.</p>
                        <input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={async (e) => {
                            const file = e.target.files?.[0];
                            if (file && confirm("Esto reemplazará todos los artículos locales. ¿Continuar?")) {
                                setIsProcessing(true);
                                const ok = await syncService.importFullVault(file);
                                if (ok) window.location.reload();
                                else alert("Error al leer archivo maestro.");
                                setIsProcessing(false);
                            }
                        }} />
                        <button onClick={() => fileInputRef.current?.click()} disabled={isProcessing} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl transition-all flex items-center justify-center gap-3">
                            {isProcessing ? <RefreshCw className="animate-spin" size={18}/> : <HardDrive size={18}/>} Seleccionar y Clonar
                        </button>
                    </div>
                </div>
