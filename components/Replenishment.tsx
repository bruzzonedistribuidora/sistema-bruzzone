...
import { Search, Plus, ShoppingCart, Truck, Send, Trash2, Mail, FileText, ChevronDown, Check, Package, X, Printer, Download, Building2, Calendar, DollarSign } from 'lucide-react';
...
const Replenishment: React.FC<ReplenishmentProps> = ({ initialItems, onItemsConsumed }) => {
  ...
  const filteredProducts = products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()) || p.internalCode.toLowerCase().includes(searchTerm.toLowerCase()));
                                  ...
                                  <div className="flex gap-2 w-full md:w-auto">
                                      <button 
                                        ...
                                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-700 text-xs font-black uppercase tracking-tighter transition-all shadow-sm">
                                          <Printer size={14}/> Ver PDF / Imprimir
                                      </button>
                                      {order.status === 'DRAFT' && (
                                          <button 
                                            onClick={() => sendOrder(order.id)}
                                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-md text-xs font-black uppercase tracking-tighter transition-all active:scale-95">
                                      ...
                                      <tbody className="divide-y divide-gray-50">
                                          {order.items.map((item, idx) => (
                                              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                  <td className="py-2 font-mono text-[10px] text-gray-500">{item.product.providerCodes[0] || item.product.internalCode}</td>
              ...
              <div className="bg-white w-full max-w-3xl h-[90vh] shadow-2xl rounded-[2.5rem] overflow-hidden flex flex-col print:h-auto print:shadow-none print:rounded-none animate-fade-in print:fixed print:inset-0 print:z-[200]">
                  ...
                  <div className="flex-1 overflow-y-auto p-12 bg-white print:overflow-visible print:p-0">
                      <div className="border border-gray-100 p-10 shadow-sm print:border-none print:shadow-none">
                          ...
                          {/* Info Proveedor */}
                          <div className="mb-10 bg-slate-50 p-8 rounded-[2rem] border border-slate-100 flex justify-between items-end">
                              <div>
                              ...
                              <tbody className="divide-y divide-gray-100">
                                  {showPrintModal.items.map((item, idx) => (
                                      <tr key={idx} className="group">
                                          <td className="px-5 py-5 font-mono text-xs text-slate-400 font-bold">{item.product.providerCodes[0] || item.product.internalCode}</td>
                          ...
                          <div className="mt-20 flex flex-col md:flex-row justify-between items-end border-t-2 border-dashed border-gray-200 pt-10 gap-8">
                              <div className="max-w-md">
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Notas y Observaciones</p>
                                  <p className="text-xs text-gray-600 leading-relaxed italic font-medium">
                                      Solicitamos confirmar recepción de este pedido y tiempo estimado de entrega a la brevedad. 
                                      Enviar factura electrónica a administracion@ferrebruzzone.com.ar citando el ID de esta orden.
                                  </p>
                              </div>
