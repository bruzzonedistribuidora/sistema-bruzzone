    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto h-full flex flex-col space-y-6 animate-fade-in bg-slate-50 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] border border-gray-200 shadow-sm gap-4 shrink-0">
            ...
            <div className="flex gap-3">
                ...
                <button onClick={() => { setModalTab('GENERAL'); setIsEditing(false); setClientForm({name: '', cuit: '', phone: '', address: '', balance: 0, limit: 100000, points: 0, portalEnabled: true, taxCondition: 'Consumidor Final', currency: 'ARS'}); setAiSources([]); setIsNewClientModalOpen(true); }} className="bg-indigo-600 text-white px-8 py-3.5 rounded-2xl flex items-center gap-3 font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all uppercase text-xs tracking-widest active:scale-95">
                            ...
                            <tr key={client.id} className="hover:bg-slate-50 transition-colors group">
                                ...
                                <td className="px-8 py-5 text-center">
                                    <div className="flex justify-center gap-2">
                                        <button onClick={() => { setIsEditing(true); setClientForm(client); setModalTab('GENERAL'); setAiSources([]); setIsNewClientModalOpen(true); }} className="p-3 bg-slate-100 text-indigo-600 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all active:scale-90" title="Editar Ficha"><Pencil size={18}/></button>
                                        ...
                                        <button onClick={() => setClients(prev => prev.filter(c => c.id !== client.id))} className="p-3 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all active:scale-90" title="Eliminar Cliente"><Trash2 size={18}/></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            <input type="text" className="flex-1 p-4 bg-white border border-gray-200 rounded-2xl focus:ring-2 focus:ring-indigo-600 outline-none font-bold" value={clientForm.cuit} onChange={e => setClientForm({...clientForm, cuit: e.target.value})} placeholder="30-..." />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input type="text" className="w-full p-4 bg-white border border-gray-200 rounded-2xl focus:ring-2 focus:ring-indigo-600 outline-none font-bold uppercase" value={clientForm.name} onChange={e => setClientForm({...clientForm, name: e.target.value.toUpperCase()})} />
                                    </div>
                                </div>
                                <div className="space-y-2">
                            ...
                            <div className="space-y-8 animate-fade-in">
                                <div className="bg-white p-8 rounded-[2rem] border border-gray-200 shadow-sm space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest ml-1">Límite de Crédito ($)</label>
                                            <input type="number" className="w-full p-4 bg-white border-2 border-transparent rounded-2xl focus:border-indigo-600 outline-none font-black text-xl" value={clientForm.limit} onChange={e => setClientForm({...clientForm, limit: parseFloat(e.target.value) || 0})} />
                                        </div>
                                    </div>
                                </div>
                            </div>
