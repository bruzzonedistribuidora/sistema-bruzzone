...
const Inventory: React.FC = () => {
  ...
  const handleSaveProduct = async () => {
      ...
      setIsSaving(true);
      try {
          // LÓGICA DE FALLBACK: Si no hay código interno pero hay barras, usar barras como SKU
                    ...
                    <div className="flex-1 overflow-auto custom-scrollbar">
                        <table className="w-full text-left border-collapse table-fixed">
                            <thead className="bg-slate-900 sticky top-0 z-20 text-[8px] uppercase font-black text-slate-300 tracking-wider">
                                <tr>
                                    <th className="w-[15%] px-4 py-3 cursor-pointer" onClick={() => requestSort('internalCodes')}>SKU / Barras</th>
          ...
          <div className="fixed inset-0 z-[300] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[95vh]">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50/50">
                              ...
                              <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-5">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                      <div>
                                          ...
                                          <input className="w-full p-2.5 bg-slate-50 border rounded-xl font-mono text-[11px] font-bold uppercase" placeholder="Opcional si tiene Barras" value={formData.internalCodes?.[0] || ''} onChange={e => setFormData({...formData, internalCodes: [e.target.value.toUpperCase()]})} />
                                      </div>
                                      <div>
                                      ...
                                      <div>
                                          <label className="text-[8px] font-black uppercase text-slate-400 block mb-1">Cant. por Bulto</label>
                                          <input type="number" className="w-full p-2.5 bg-slate-50 border rounded-xl font-black text-[11px] text-center" value={formData.purchasePackageQuantity || 1} onChange={e => setFormData({...formData, purchasePackageQuantity: parseInt(e.target.value) || 1})} />
                                      </div>
                                  </div>
                              </div>
                          </div>
                      )}
                      {/* El resto de los tabs se mantienen igual */}
                      {modalTab === 'PRICING' && (
                          <div className="space-y-6 animate-fade-in">
                              <div className="bg-white p-6 rounded-2xl border border-slate-200 grid grid-cols-1 md:grid-cols-12 gap-8">
                                  <div className="md:col-span-5 space-y-6">
                                      ...
                                      <div>
                                          <label className="text-[9px] font-black text-slate-400 block mb-2">Costo Lista Bruto</label>
                                      ...
                                      <div className="space-y-3">
                                          <label className="text-[9px] font-black text-slate-400 block ml-1">Bonificaciones (%)</label>
                      ...
                      {/* Otros tabs resumidos para brevedad */}
                  </div>
