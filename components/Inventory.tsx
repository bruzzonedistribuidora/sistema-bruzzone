...
import { 
    ...
    Download, UploadCloud, RefreshCw, Phone, Mail, UserSearch
} from 'lucide-react';
...
const Inventory: React.FC = () => {
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (event) => {
          ...
          if (inventoryTab === 'BRANDS') {
            ...
            const newProvs: Provider[] = [];
            items.forEach(name => {
                if (!currentNames.has(name)) {
                    newProvs.push({ id: `p-${Date.now()}-${Math.random()}`, name, cuit: '', contact: '', balance: 0, defaultDiscounts: [0,0,0] });
                    currentNames.add(name);
                }
            });
            setProviders([...providers, ...newProvs]);
            alert(`Importación finalizada. Se agregaron ${newProvs.length} proveedores nuevos.`);
        }
      };
  ...
  const filteredProviders = useMemo(() => {
    const term = searchTerm.toLowerCase().trim();
    return providers.filter(p => p.name.toLowerCase().includes(term) || p.cuit.includes(term)).sort((a,b) => a.name.localeCompare(b.name));
    ...
    <div className="p-4 h-full flex flex-col max-w-full mx-auto space-y-3 bg-slate-50 overflow-hidden">
      ...
      <div className="flex justify-between items-end bg-white p-4 rounded-2xl border border-gray-200 shadow-sm shrink-0">
          <div>
              ...
              <div className="flex mt-3 bg-slate-100 p-1 rounded-xl gap-1">
                  ...
                  <button onClick={() => { setInventoryTab('PROVIDERS'); setSearchTerm(''); }} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${inventoryTab === 'PROVIDERS' ? 'bg-white text-slate-900 shadow-sm' : 'text-gray-400 hover:text-slate-600'}`}>Proveedores ({providers.length})</button>
              </div>
          </div>
          ...
          <div className="flex gap-3">
              ...
              {inventoryTab === 'PRODUCTS' ? (
                  ...
                  <button onClick={() => setIsQuickAddOpen(inventoryTab === 'BRANDS' ? 'BRAND' : inventoryTab === 'CATEGORIES' ? 'CATEGORY' : 'PROVIDER')} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black shadow-xl flex items-center gap-3 transition-all uppercase text-xs tracking-widest hover:bg-slate-800">
                      <Plus size={18} /> Nueva {inventoryTab === 'BRANDS' ? 'Marca' : inventoryTab === 'CATEGORIES' ? 'Categoría' : 'Entidad'}
                  </button>
              )}
          </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-2 shrink-0">
            <div className="relative group">
                ...
                <input type="text" placeholder={`Buscar en ${inventoryTab.toLowerCase()}...`} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-transparent rounded-lg text-xs font-bold outline-none focus:bg-white focus:border-indigo-100 transition-all uppercase" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
      </div>
            ...
            {inventoryTab === 'PROVIDERS' && (
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
                    {filteredProviders.map(p => (
                        <div key={p.id} className="bg-white border border-slate-200 p-6 rounded-2xl flex flex-col group hover:border-indigo-400 hover:shadow-xl transition-all relative overflow-hidden">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => { if(confirm('¿Eliminar proveedor?')) setProviders(providers.filter(x => x.id !== p.id)) }} className="p-2 text-slate-300 hover:text-red-500 transition-all"><Trash2 size={16}/></button>
                                </div>
                            </div>
                            <h4 className="font-black text-slate-800 uppercase text-sm leading-tight mb-1">{p.name}</h4>
                            <p className="text-[10px] text-gray-400 font-mono font-bold">{p.cuit || 'SIN CUIT'}</p>
                            ...
                            <div className="mt-6 flex justify-between items-end">
                                <div>
                                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Saldo</p>
                                    <p className={`text-lg font-black tracking-tighter leading-none ${p.balance > 0 ? 'text-red-500' : 'text-green-500'}`}>${p.balance.toLocaleString('es-AR')}</p>
                                </div>
                                <button className="text-[9px] font-black text-indigo-600 uppercase flex items-center gap-1 hover:underline">Auditar Cta <ChevronRight size={12}/></button>
                            </div>
                        </div>
                    ))}
                </div>
