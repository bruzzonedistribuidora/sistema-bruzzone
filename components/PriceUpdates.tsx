...
// Listas de precios estándar del sistema (Garantía de contenido inicial)
...
const PriceUpdates: React.FC = () => {
    ...
    // --- ESTADO DE LISTAS CON FALLBACK ABSOLUTO ---
    ...
    // Lógica para forzar reconstrucción si se borran accidentalmente
    ...
    const productsInList = useMemo(() => {
        ...
        return filtered.map(p => {
            ...
            const margin = viewingListDetail.type === 'BASE' ? (p.profitMargin || 0) : (viewingListDetail.fixedMargin || 0);
    ...
    const handleSaveList = () => {
        ...
        setPriceLists(prev => {
            if (editingList) {
                ...
                return [...prev, { ...listForm, id: Date.now().toString(), active: true, type: listForm.type || 'CUSTOM' } as PriceList];
            }
        });
        ...
        <div className="p-6 h-full flex flex-col space-y-6 bg-slate-50 overflow-hidden font-sans">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 px-2">
                        ...
                        <div className="md:col-span-2 flex items-center justify-end gap-3">
                            ...
                            <button 
                                ...
                                className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-2xl hover:bg-slate-800 transition-all flex items-center gap-3 active:scale-95">
                            ...
                            <div key={list.id} className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all flex flex-col justify-between group h-fit relative overflow-hidden animate-fade-in">
                                ...
                                <div>
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100">
                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Cálculo de Precio</p>
                                ...
                                {productsInList.length === 0 ? (
                                    ...
                                    <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                                        ...
                                        <td className="px-10 py-6 text-center">
                                            <span className="bg-slate-100 px-4 py-1.5 rounded-xl font-black text-slate-600 border border-slate-200">+{p.appliedMargin || 0}%</span>
                                        </td>
            ...
            {isListModalOpen && (
                <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 animate-fade-in">
                    <div className="bg-white rounded-[4rem] shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
                        ...
                        <div className="p-12 space-y-12">
                            <div className="space-y-10">
                                <div>
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 ml-2">Nombre Identificatorio</label>
                                    <input type="text" className="w-full p-6 bg-slate-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-indigo-600 outline-none font-black text-slate-800 uppercase shadow-inner text-lg" value={listForm.name} onChange={e => setListForm({...listForm, name: e.target.value.toUpperCase()})} autoFocus />
                                </div>
